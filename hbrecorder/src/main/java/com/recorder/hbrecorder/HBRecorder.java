...
public class HBRecorder implements MyListener {
    ...
    public boolean isBusyRecording() {
        ...
        Log.d("manageerrr", String.valueOf(manager));
        if (manager != null) {
            for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
                Log.d("integerr", String.valueOf(Integer.MAX_VALUE));
                Log.d("servicename",ScreenRecordService.class.getName());
                Log.d("servicename2",service.service.getClassName());
                if (ScreenRecordService.class.getName().equals(service.service.getClassName())) {
                    Log.d("busyrecording!null", "niptaaa");
                    return true;
                }
            }
        }
        Log.d("busyrecording??", "ptaaa");
        return false;
    }
